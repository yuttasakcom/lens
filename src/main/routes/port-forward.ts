...
class PortForward {
  ...
  static getPortforward(forward: {kind: string; name: string; namespace: string; port: string}) {
    return PortForward.portForwards.find((pf) => {
      return pf.kind == "service" && pf.name == forward.name && pf.namespace == forward.namespace && pf.port == forward.port
    })
  }
  ...
  public async start() {
    ...
    tcpPortUsed.waitUntilUsed(this.localPort, 500, 10000)
  }
